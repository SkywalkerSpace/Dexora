...
class RDTRunner(
    ...
    def conditional_sample(self, lang_cond, lang_attn_mask, img_cond, 
                           ...
                           warm_start_t=None, warm_start_action=None):
        '''
        ...
        warm_start_t: Optional diffusion timestep from which to start sampling.
            When provided, warm_start_action is assumed to already be the low-
            noise state at this timestep, and we only run the reverse steps
            from this point downward.
        warm_start_action: Optional initial noised action sequence in the same
            shape as the denoised action. This is typically produced by calling
            scheduler.add_noise(gt_action, noise, t_start) before sampling.
        ...
        if warm_start_action is not None:
            if warm_start_t is None:
                raise ValueError("warm_start_t must be provided when warm_start_action is set")
            warm_start_action = warm_start_action.to(device=device, dtype=dtype)
            if warm_start_action.shape != (state_traj.shape[0], self.pred_horizon, self.action_dim):
                raise ValueError(
                    "warm_start_action must have shape "
                    f"({state_traj.shape[0]}, {self.pred_horizon}, {self.action_dim}), "
                    f"got {tuple(warm_start_action.shape)}"
                )

            self.noise_scheduler_sample.set_timesteps(self.num_inference_timesteps)
            all_timesteps = self.noise_scheduler_sample.timesteps
            warm_start_t = int(warm_start_t)
            ...
            # Set step values
            self.noise_scheduler_sample.set_timesteps(self.num_inference_timesteps)
            valid_timesteps = self.noise_scheduler_sample.timesteps
    ...
    def predict_action(self, lang_tokens, lang_attn_mask, img_tokens, state_tokens,
        ...
        ctrl_freqs: (batch_size,), control frequency for each sample.
        warm_start_t: Optional diffusion timestep to begin denoising from.
        warm_start_action: Optional GT/noisy GT action used to initialize the
            reverse process near the target trajectory.
