...
class DexoraPolicy:
    ...
    def get_action(
        self,
        obs: dict,
        warm_start_t: Optional[int] = None,
        warm_start_action: Optional[np.ndarray] = None,
    ) -> np.ndarray:
        """
        ...
        Optional warm-start sampling:
          * ``warm_start_t``: integer diffusion timestep to start denoising from.
          * ``warm_start_action``: already noised action chunk used as the starting
            point for the reverse process. Shape can be ``[chunk_size, state_dim]``
            or ``[1, chunk_size, state_dim]``.
        """
        # ---- 1. Language ---------------------------------------------------
        ...
        if warm_start_action is not None:
            warm_start_action = np.asarray(warm_start_action, dtype=np.float32)
            if warm_start_action.ndim == 1:
                warm_start_action = warm_start_action[None, :]
            elif warm_start_action.ndim == 3 and warm_start_action.shape[0] == 1:
                warm_start_action = warm_start_action[0]
            if warm_start_action.shape[-1] != self.cfg.state_dim:
                raise ValueError(
                    f"warm_start_action last dim={warm_start_action.shape[-1]} does not match state_dim={self.cfg.state_dim}"
                )
            warm_start_action = torch.as_tensor(warm_start_action, device=self.device, dtype=self.cfg.dtype)
            if warm_start_action.ndim == 2:
                warm_start_action = warm_start_action[None, :, :]
        else:
